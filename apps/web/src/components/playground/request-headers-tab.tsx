  ...
  return (
    <div className="space-y-3 animate-in fade-in duration-200">
      <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-[#fdf9f3] dark:bg-slate-950">
        ...
        <div className={`flex items-center gap-2.5 py-2.5 px-3 border-b border-slate-200 dark:border-slate-800/60 text-xs transition-opacity ${!useExternalApiKey || isChecked("X-AI-API-KEY") ? "opacity-100" : "opacity-45"}`}>
          <div className="shrink-0">
            <input
              type="checkbox"
        ...
        {/* HEADER 6: X-AI-BASE-URL */}
        <div className={`flex items-center gap-2.5 py-2.5 px-3 text-xs transition-opacity ${!useExternalApiKey || isChecked("X-AI-BASE-URL") ? "opacity-100" : "opacity-45"}`}>
          <div className="shrink-0">
            <input
              type="checkbox"
              checked={!useExternalApiKey ? true : isChecked("X-AI-BASE-URL")}
              disabled={!useExternalApiKey}
              readOnly={!useExternalApiKey}
              onChange={() => useExternalApiKey && toggleCheck("X-AI-BASE-URL")}
              className={`rounded border-slate-300 dark:border-slate-700 text-[#e1b329] focus:ring-0 ${!useExternalApiKey ? "bg-slate-200 dark:bg-slate-800 cursor-not-allowed opacity-80" : "bg-white dark:bg-slate-900 cursor-pointer"}`}
            />
          </div>
          <div className="w-4/12 shrink-0">
          ...
          <div className="flex-1">
            <input
              ...
              value={aiBaseUrl}
              readOnly={!useExternalApiKey}
              disabled={!useExternalApiKey || !isChecked("X-AI-BASE-URL")}
              onChange={(e) => useExternalApiKey && setAiBaseUrl?.(e.target.value)}
              className={`w-full border border-slate-300 dark:border-slate-700 rounded-lg px-2.5 py-1.5 text-xs text-sky-700 dark:text-sky-300 font-mono focus:outline-none focus:border-[#e1b329] ${!useExternalApiKey
                ? "bg-slate-100 dark:bg-slate-900/60 cursor-not-allowed opacity-80"
                : isChecked("X-AI-BASE-URL")
                  ? "bg-white dark:bg-slate-900"
                  : "bg-slate-100/60 dark:bg-slate-900/40 opacity-50 cursor-not-allowed"
                }`}
              placeholder="e.g. https://api.groq.com/openai/v1 (optional)"
            />
          </div>
        </div>
      </div>
