...
# Router for common OAuth reverse-proxy and redirect URI aliases to avoid 404 RESOURCE_NOT_FOUND
alias_router = APIRouter(tags=["Google Authentication Aliases"])
...
async def exchange_google_code_alias(
    ...
    return await exchange_google_code(payload=payload, db=db)
...
async def auth_callback_fallback(
    ...
    """
    Fallback handler when Google OAuth redirects directly to /auth/callback on the API server,
    or when FRONTEND_URL was misdirected to the API host.
    """
    if state and code:
        return await google_auth_callback(
            code=code,
    ...
    query_parts = []
    if code:
        query_parts.append(f"code={urllib.parse.quote(code)}")
    if state:
        query_parts.append(f"state={urllib.parse.quote(state)}")
    if error:
        query_parts.append(f"error={urllib.parse.quote(error)}")
    if error_description:
        query_parts.append(f"error_description={urllib.parse.quote(error_description)}")

    qs = f"?{'&'.join(query_parts)}" if query_parts else ""
    return RedirectResponse(f"{frontend_url}/auth/callback{qs}", status_code=status.HTTP_302_FOUND)
