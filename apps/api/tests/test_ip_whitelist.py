    ...
    with patch("callcraft_api.db.repository.Repository.verify_api_credential", new_callable=AsyncMock) as mock_verify:
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
            ...
            res = await ac.post(
                "/v1/call/usr_test",
                json={"prompt": "test"},
                headers={"Authorization": "Bearer call_sk_live_test", "X-Forwarded-For": "203.0.113.50"},
            )
            assert res.status_code == 403
            assert "not authorized for this API key" in res.json()["detail"]
            ...
            with patch("callcraft_api.services.redis_cache.redis_service.get_spec", new_callable=AsyncMock) as mock_redis:
                ...
                res_allowed = await ac.post(
                    "/v1/call/usr_test",
                    json={"prompt": "test"},
                    headers={"Authorization": "Bearer call_sk_live_test", "X-Forwarded-For": "10.0.0.15"},
                )
